...
export interface ICarOwnerService{
    ...
    loginCarOwner(email:string,password:string):Promise<{carOwner:ICarOwner|null,refreshToken:string,ownerAccessToken:string}>
    ...
    changePassword(ownerId: string, passwordDetails: { oldPassword: string; newPassword: string }): Promise<{success:boolean, message: string }>
    logoutCarOwner(refreshToken: string): Promise<void>
    loginOwnerGoogle(fullName: string, email: string,googleId: string, image: string, provider: string, role?: string):Promise<{ownerAccessToken:string,refreshToken:string,carOwner:ICarOwner|null}>

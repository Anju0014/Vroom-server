...
export interface ICustomerService{
    ...
    loginCustomer(email:string,password:string):Promise<{customer:ICustomer|null,refreshToken:string,customerAccessToken:string}>
    ...
    changePassword(customerId: string, passwordDetails: { oldPassword: string; newPassword: string }): Promise<string>
    
    logoutCustomer(refreshToken: string): Promise<void>
    loginCustomerGoogle(fullName: string, email: string, image: string, provider: string, role?: string) :Promise<{customerAccessToken:string,refreshToken:string,customer:ICustomer|null}>

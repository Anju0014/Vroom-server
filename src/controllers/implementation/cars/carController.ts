...
// import { ICarService } from '../../services/interfaces/cars/ICarService';
...
//       const total = await this.carService.getCarsCount(ownerId);
//       res.status(StatusCode.OK).json({
//         success: true,
//         cars: cars.map(carMapper.toDTO),
//         total,
//       });
//     } catch (error) {
//       this.handleError(res, error, StatusCode.INTERNAL_SERVER_ERROR);
...
//         res.status(StatusCode.BAD_REQUEST).json({ success: false, message: 'unavailableDates must be an array of strings' });
//         return;
//       }
...
//         car: carMapper.toDTO(deletedCar),
//       });
//     } catch (error) {
//       this.handleError(res, error, StatusCode.INTERNAL_SERVER_ERROR);
//     }
...
//       const carId = req.params.id;
//       const ownerId = req.userId;
//       const { carName, brand, year, fuelType, rcBookNo, expectedWage, location, images, videos, available, carType, rcBookProof, insuranceProof } = req.body;
//       if (!ownerId) {
//         res.status(StatusCode.UNAUTHORIZED).json({ success: false, message: MESSAGES.ERROR.UNAUTHORIZED });
//         return;
//       }
...
//         return;
//       }
//       const cars = await this.carService.getAllCars(page, limit, {
//         search,
//         minPrice,
//         maxPrice,
//         carType,
...
//         endDate,
//       });
//       const total = await this.carService.getCarsCount({ search, minPrice, maxPrice, carType, location, startDate, endDate });
//       res.status(StatusCode.OK).json({
//         success: true,
//         data: cars.map(carMapper.toDTO),
//         total,
...
export class CarController {
  ...
  async uploadCar(req: CustomRequest, res: Response): Promise<void> {
    try {
      if (!req.user || req.user.role !== 'carOwner') {
        res.status(StatusCode.UNAUTHORIZED).json({
          success: false,
          message: MESSAGES.ERROR.UNAUTHORIZED,
        });
